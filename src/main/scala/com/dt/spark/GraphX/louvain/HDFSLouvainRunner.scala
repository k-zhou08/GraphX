...
class HDFSLouvainRunner(minProgress:Int,progressCounter:Int,outputdir:String) extends LouvainHarness(minProgress:Int,progressCounter:Int){
  ...
  override def saveLevel(sc:SparkContext,level:Int,q:Double,graph:Graph[VertexState,(Long,Long)]) = {
      ...
      sc.parallelize(qValues, 1).saveAsTextFile(outputdir+"/qvalues"+qValues)
  }
